...
function buildXml(data, identity, totals) {
  ...
  return `<?xml version="1.0" encoding="UTF-8"?>\n<${root} xmlns="${namespace}" xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2" xmlns:cbc="urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2"><cbc:UBLVersionID>2.1</cbc:UBLVersionID><cbc:CustomizationID>2.0</cbc:CustomizationID><cbc:ID>${data.series}-${data.number}</cbc:ID><cbc:IssueDate>${data.issueDate}</cbc:IssueDate>${docCodeNode}<cbc:DocumentCurrencyCode>PEN</cbc:DocumentCurrencyCode>${reference}${supplierXml(data.issuer)}${customerXml(data.customer, identity)}${taxXml(totals)}<cac:${legalNode}><cbc:LineExtensionAmount currencyID="PEN">${money(totals.taxable + totals.exonerated)}</cbc:LineExtensionAmount><cbc:TaxInclusiveAmount currencyID="PEN">${money(totals.total)}</cbc:TaxInclusiveAmount><cbc:${totalNode} currencyID="PEN">${money(totals.total)}</cbc:${totalNode}></cac:${legalNode}>${totals.lines.map((line) => lineXml(line, root)).join("")}</${root}>`;
}
...
function buildSunatDraft(input) {
  ...
  return {
    ...
    localDigest: digest,
    xml,
    warnings: ["XML sin firma digital.", "No se realizó conexión con SUNAT.", "Validar contra XSD y reglas SUNAT antes de habilitar envío."],
  };
}
